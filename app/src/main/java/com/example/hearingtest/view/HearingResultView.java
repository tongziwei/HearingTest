package com.example.hearingtest.view;
...
import com.example.hearingtest.utils.DensityUtils;
...
public class HearingResultView extends View {
    ...
    String[] xStr = {"1k", "2k", "4k", "8k","500","250"};
    /** X轴刻度数 */
    private int xLineCount = 6;
    /** Y轴刻度数 */
    private int yLineCount = 11;
    /** Y轴最大值 */
    private int yMaxValue = 90;
    /** Y轴最小值 */
    ...
    public HearingResultView(Context context, int modly) {
        super(context);
        this.modly = modly;
    ...
    public HearingResultView(Context context, AttributeSet attrs) {
        super(context, attrs);
        this.mContext = context;
        init();
    }

    public HearingResultView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init() {
        ...
        margin.updateMargin(DensityUtils.dip2px(mContext, 20),
                ...
                DensityUtils.dip2px(mContext, 12),
                DensityUtils.dip2px(mContext, 20));// 初始化视图边距
        // 初始化数据
...
//                    - DensityUtils.dip2px(mContext, 2), recPaint);
            canvas.drawText(xStr[j], xAxisPointArr[j], this.getHeight() - margin.getBottom()
