...
public class AudioTrackManager {
    ...
    public void start(int channel) {
        ...
        if (Hz > 0) {
            waveLen = RATE / Hz;
            length = waveLen * Hz*20;  //length = waveLen * Hz*20测持续的正弦波音
            audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC,// 类型
                    RATE,// 频率
            ...
            switch (channel) {
                case LEFT:
                   // audioTrack.setStereoVolume(volume, 0f);
                    audioTrack.setStereoVolume(AudioTrack.getMaxVolume(), 0f);
                    break;
                case RIGHT:
                    audioTrack.setStereoVolume(0f, AudioTrack.getMaxVolume());
                    break;
                case DOUBLE:
